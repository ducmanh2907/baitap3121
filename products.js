...
const { verifyToken, isModerator, isAdmin } = require('../middleware/auth');
...
router.post('/', verifyToken, isModerator, async function(req, res, next) {
  try {
    let body = req.body;
...
// Moderator access required
router.put('/:id', verifyToken, isModerator, async function(req, res, next) {
  try {
    let id = req.params.id;
    let product = await productSchema.findById(id);
    if(product){
